    ...
    <div className='grid grid-cols-1 md:grid-cols-2 gap-5'>
      ...
      {items && items.length > 0 ? (
        items.map((item) => (
          ...
          <img src="/default-image.png" className='w-32' alt="Default" />
        )}
        <div className="pr-12 pl-2 gap-6 flex flex-col">
            <h1 className='text-2xl font-black border-b-2 border-black'>{item.lokasi}</h1>
            <div className="flex flex-col h-full">
            <p className='text-md'>{item.description}</p>
            <p className='text-sm flex items-end'>Tipe Kerusakan : {item.kerusakan}</p>
            </div>
