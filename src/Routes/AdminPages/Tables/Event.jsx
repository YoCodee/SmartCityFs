...
function EventTable() {
    ...
    const fetchTable = async () => {
        try {
            const response = await axios.get("https://web-city-server.vercel.app/api/event");
    ...
    const updateBookingStatus = async (reportId) => {
        try {
            const response = await axios.put(`https://web-city-server.vercel.app/api/event/${reportId}`);
    ...
    return (
        ...
        <h1 className="text-3xl font-bold mb-6 text-center">Laporan Event</h1>
            ...
            <table id="perbaikanTable" className="min-w-full bg-white border border-gray-300">
                <thead className="bg-gray-100 border-b">
                    <tr>
                        ...
                        <th className="px-4 py-2 text-left text-gray-600">Nama Organisasi</th>
                        <th className="px-4 py-2 text-left text-gray-600">Visi Misi</th>
                ...
                <tbody>
                    {tableData.map((report) => (
                        <tr key={report._id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="px-4 py-2">{report.namaOrganisasi}</td>
                            <td className="px-4 py-2">{report.VisiMisi}</td>
