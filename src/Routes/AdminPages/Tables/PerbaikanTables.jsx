...
function PerbaikanTables() {
    ...
    const fetchTable = async () => {
        try {
            const response = await axios.get("http://localhost:3000/api/perbaikan");
    ...
    const updateBookingStatus = async (reportId) => {
        try {
            const response = await axios.put(`http://localhost:3000/api/perbaikan/approve/${reportId}`);
    ...
    <h1 className="text-3xl font-bold mb-6 text-center">Laporan Perbaikan Jalan</h1>
        ...
        <table id="perbaikanTable" className="min-w-full bg-white border border-gray-300">
            <thead className="bg-gray-100 border-b">
                <tr>
                    ...
                    <th className="px-4 py-2 text-left text-gray-600">Kerusakan</th>
            ...
            <tbody>
                {tableData.map((report) => (
                    <tr key={report._id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2">{report.kerusakan}</td>
...
export default PerbaikanTables;
