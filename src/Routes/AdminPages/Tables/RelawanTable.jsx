...
function RelawanTable() {
    ...
    <h1 className="text-3xl font-bold mb-6 text-center">Laporan Relawan</h1>
    <div className="overflow-x-auto">
        <table id="perbaikanTable" className="min-w-full bg-white border border-gray-300">
            <thead className="bg-gray-100 border-b">
                <tr>
                    <th className="px-4 py-2 text-left text-gray-600">ID</th>
                    <th className="px-4 py-2 text-left text-gray-600">Report Type</th>
                    <th className="px-4 py-2 text-left text-gray-600">Waktu Dibuat</th>
                    <th className="px-4 py-2 text-left text-gray-600">Waktu Selesai</th>
        ...
        <tr key={report._id}>
            <td>{report._id}</td>
            <td>{report.reportType || "Unknown Type"}</td> {/* Modify as needed */}
            <td>{new Date(report.assignedAt).toLocaleString()}</td>
            <td>{report.completedAt ? new Date(report.completedAt).toLocaleString() : "Not Completed"}</td>
            <td>
